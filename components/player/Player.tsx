...
/**
 * 플레이어 아바타 + 구면 이동 로직.
 * - 입력은 useKeyboardInput 의 ref 에서 읽는다.
 * - 위치/방향은 transform(공유 ref)에 직접 쓰고, 그 값으로 group 을 배치한다.
 * - useFrame 안에서 React state 를 만들지 않는다.
 */
export default function Player({ transform }: PlayerProps) {
  ...
  useFrame((_, rawDelta) => {
    ...
    // 전/후진 — 표면 위 대원 이동
    if (moveDir !== 0) {
      const arc = (moveDir * worldConfig.moveSpeed * delta) / worldConfig.surfaceRadius;
    ...
    // 이동 중 bobbing: 목표값을 만들고 부드럽게 수렴.
    if (moveDir !== 0) bobPhase.current += delta * 9;
    const bobTarget = moveDir !== 0 ? Math.sin(bobPhase.current) * 0.03 : 0;
